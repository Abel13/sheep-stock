...
export default function SaleScreen() {
  ...
  return (
    <YStack padding="$4" flex={1} backgroundColor="$background">
      <Text htmlFor='customer' fontSize={12}>Cliente</Text>
      <Input
      ...
      {showSearchResults && search && (
        ...
        >
          {isLoading ? (
            <Text paddingVertical="$2" paddingHorizontal="$2">Carregando produtos...</Text>
          ) : (
              <FlatList
                data={products}
                ...
                renderItem={({item}) => (
                  ...
                  >
                    <YStack flex={1}>
                      <Text fontWeight='400' fontSize={10} color={'gray'}>{item.product_code}</Text>
                      <Text fontWeight='500'>{item.product_name}</Text>
                      <Text marginTop='$2'>Preço de venda: R$ {(item.sale_price || 0).toFixed(2)}</Text>
                    </YStack>
                    <YStack justifyContent='center' alignItems='center'>
                      <Text fontWeight={'300'} fontSize={12}>ESTOQUE</Text>
                      <Text fontSize={22} fontWeight={'500'}>{item.stock_quantity}</Text>
                    </YStack>
                  </ListItem>
                )}
              />
      ...
      <Label fontSize={'$7'} marginTop={'$3'}>Produtos:</Label>
      <FlatList
        data={selectedProducts}
        keyExtractor={(item) => item.product_code}
        ItemSeparatorComponent={() => <Spacer size={5} />}
        ListEmptyComponent={() =>
          <Card justifyContent='center' alignItems='center' padding={20} bordered>
        ...
        renderItem={({ item }) => (
          ...
          >
            ...
            <XStack gap="$2" alignItems="center">
              <Button onPress={() => decrementQuantity(item.product_code)}>-</Button>
              <Text>{item.quantity.toString().padStart(2, '0')}</Text>
              <Button onPress={() => incrementQuantity(item.product_code)}>+</Button>
            </XStack>
          </ListItem>
        )}
      />
